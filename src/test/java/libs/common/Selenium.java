...
public class Selenium {
	...
	public boolean isElementNotPresent(WebDriver driver, By byLocator)
			throws Exception {
		try {
			 boolean visible = driver.findElement(byLocator).isDisplayed();
		        boolean result = !visible;
		        return result;
		} catch (Exception ex) {
			throw new Exception(
					"isElementNotExisted - Failed - Exception occurs: " + ex);
		}
	}
...
//			mobileClientDriver.swipe(x, y, x1, y1, 1000);
	...
	public void longPress(WebDriver mobileClientDriver, By element) throws Exception {
		logger.info("longPress - starting ...\n");
		logger.info("byLocator:" + element + "\n");
		try{			
			
			Actions builder = new Actions(mobileClientDriver);
			WebElement el = mobileClientDriver.findElement(element);
			builder.clickAndHold(el).build().perform();	
			Thread.sleep(1000);
			builder.release();
			
		} catch (Exception ex) {
			logger.error("longPress - Failed - Exception occurs:" + ex + "\n");			
			throw new Exception("longPress - Failed - Exception occurs:" + ex);
		}
		logger.info("longPress - completed ...\n");
