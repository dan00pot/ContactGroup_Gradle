...
//				clickCancelSearch(winClientDriver);
					...
					if(!selenium.isElementExisted(winClientDriver, windowsClient.CONTACT_LIST_CONTACT_NAME(name))){
						...
						driverMgnt.setFailedWinClientDriver(winClientDriver);
						throw new Exception("Verify" +name +"is removed failure - FAILED");
					}
...
//			selenium.clickElement(winClientDriver, windowsClient.ADD_CONTACT_SCREEN_ADD_CONTACT_BTN);}
	...
	public boolean SearchContact(WindowsDriver winRootDriver, String input, String result) throws Exception{
		...
		try {
			sikuli.click("imgs/WinClient-SearchBar.PNG");
			sikuli.type(null, input);
			Thread.sleep(1000);
			
			winClientDriver = createWinHandleDriver(winRootDriver, "Avaya Equinox");
			
			selenium.doubleClickElement(winClientDriver, windowsClient.mainWindowsContactsTabUserLocatorsAfterSearch(input));
			//selenium.clickElement(winClientDriver, windowsClient.ADD_CONTACT_SCREEN_ADD_CONTACT_BTN);
//			Thread.sleep(2000);
//			clickCancelSearch(winClientDriver);
			
			if(selenium.isElementExisted(winClientDriver, windowsClient.CONTACT_IN_CONTACT_LIST(input))) {
				s=true;
			}
